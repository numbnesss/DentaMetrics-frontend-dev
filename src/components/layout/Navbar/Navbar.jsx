...
function UserMenu() {
  ...
  const roleLabel = user?.role === 'admin' ? 'Администратор' : 'Обычный пользователь';

  return (
    <div className={styles.userWrap} ref={ref}>
      <button className={styles.avatar} onClick={() => setOpen((v) => !v)} title={user?.name}>
        {user?.initials}
      </button>
    ...
    <header className={styles.navbar}>
      <div className={styles.inner}>
        <div className={styles.logo}>
          ...
          <span className={styles.logoText}>DentaMetrics</span>
        </div>
